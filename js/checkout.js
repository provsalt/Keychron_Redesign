    ...
    setupShippingOptions() {
        ...
        shippingOptions.forEach(option => {
            option.addEventListener('click', () => {
                ...
                this.updateOrderSummary({ shipping: parseFloat(price.replace('$', '')) });
            });
        });
    }
    ...
    updateOrderSummary({ shipping = 5.99, discount = 0 } = {}) {
        ...
        if (discount > 0) {
            if (!elements.discount) {
                ...
                `;
                this.summaryEl.querySelector('.summary-content').insertBefore(
                    discountLine,
                    this.summaryEl.querySelector('.total')
                );
            } else {
        ...
        this.form.addEventListener('submit', async (e) => {
            ...
            const orderData = {
                ...
                payment: {
                    ...
                    card: this.state.paymentMethod === 'card' ? {
